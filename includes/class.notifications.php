...
class WoocommerceIR_Notification_SMS
{
    ...
    public function __construct()
    {
        if (ps_sms_options('enable_notif_sms_main', 'sms_notif_settings', 'off') == 'on') {
            ...
            add_action('woocommerce_product_sms', array($this, 'productPageASANAKCustomTabsPanel' ));
     ...
     * @return void
     */
    public function productPageASANAKCustomTabsPanel($thepostid)
    {
        ...
        echo '<hr/>';
        echo '<div class="ASANAK-woo-tabs-hidden-how-to-info" style="display: none;">
			...
			<div class="dashicons dashicons-editor-help ASANAK-tabs-how-to-toggle" title="راهنمایی"></div>';
    ...
    {
        wc_enqueue_js(
            'var ASANAK_Woo_SMS = jQuery.noConflict();
			ASANAK_Woo_SMS(document).ready(function(){
				ASANAK_Woo_SMS("#submit_result").hide();
				ASANAK_Woo_SMS("#sms_news_text").hide();
				ASANAK_Woo_SMS(".sms_qroup_check_label").hide();
				ASANAK_Woo_SMS(".sms_qroup_check").hide();
				ASANAK_Woo_SMS("#sms_news_btn").hide();
				ASANAK_Woo_SMS("#sms_news_check").change(function(){
					if( ASANAK_Woo_SMS("#sms_news_check:checked").val() ) {
						ASANAK_Woo_SMS("#sms_news_text").fadeIn(); 
						ASANAK_Woo_SMS("#sms_news_btn").fadeIn(); 	
						ASANAK_Woo_SMS("#submit_result").fadeIn(); 	
						ASANAK_Woo_SMS(".sms_qroup_check_label").fadeIn(); 	
						ASANAK_Woo_SMS(".sms_qroup_check").fadeIn(); 	
					}						
					else{
						ASANAK_Woo_SMS("#sms_news_text").fadeOut();
						ASANAK_Woo_SMS("#sms_news_btn").fadeOut();
						ASANAK_Woo_SMS("#submit_result").fadeOut();
						ASANAK_Woo_SMS(".sms_qroup_check_label").fadeOut();
						ASANAK_Woo_SMS(".sms_qroup_check").fadeOut();
					}
				});
			});
			ASANAK_Woo_SMS( document ).on( "click", "#sms_news_btn", function() {
				ASANAK_Woo_SMS("#submit_result").html( "<img src=\"'.PS_WOO_SMS_PLUGIN_PATH.'/assets/images/ajax-loader.gif\" />" );
				var sms_number = ASANAK_Woo_SMS("#sms_news_text").val();
				var sms_group = [];
				ASANAK_Woo_SMS(".sms_qroup_check:checked").each(function(i){
					sms_group[i] = ASANAK_Woo_SMS(this).val();
				});
				ASANAK_Woo_SMS.ajax({
					...
					success : function( response ) {
						ASANAK_Woo_SMS("#submit_result").html( response );
					}
				});
				return false;
			});'
    ...
    function saveNumbersToProductMeta()
    {
        ...
        if (defined('DOING_AJAX') && DOING_AJAX) {
            if (!$sms_number || strlen($sms_number) < 1) {
                ...
                $old_meta = get_post_meta($product_id, '_ASANAK_sms_notification', true) ? get_post_meta($product_id, '_ASANAK_sms_notification', true) : '';
                ...
                if (empty($old_meta) || (!empty($old_meta) && strpos($new_meta, $old_meta) === false)) {
                    if (strlen($new_meta)> 2) {
                        ...
                        update_post_meta($product_id, '_ASANAK_sms_notification', $meta);
    ...
    function sendSmsWhenIsOnsale($product)
    {
        ...
        if ((get_post_meta($product_id, '_enable_onsale', true) == 'on') || (ps_sms_options('enable_onsale', 'sms_notif_settings', '') == 'on')) {
            ...
            $product_metas = get_post_meta($product_id, '_ASANAK_sms_notification', true) ? get_post_meta($product_id, '_ASANAK_sms_notification', true) : '';
    ...
    function sendSmsWhenIsLowStock($product)
    {
        ...
        if (ps_sms_options('enable_product_admin_sms', 'sms_product_admin_settings', 'on') == 'on') {
            $admin_datas = maybe_unserialize(get_post_meta($product_id, '_ASANAK_woo_products_tabs', true));
