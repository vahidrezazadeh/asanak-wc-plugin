...
class WoocommerceIR_Order_SMS
{
    ...
    public function sendSmsWhenOrderStatusChanged($order_id, $old_status, $new_status)
    {
        ...
        $order_page = (!empty($_POST['shop_order_asanak']) && $_POST['shop_order_asanak'] == 'true') ? true : false;
        ...
        if ($this->buyerCanGetPM($order_id, $new_status)) {
            $buyer_response_sms = WoocommerceIR_Gateways_SMS::init()->sendAsanak($buyer_sms_data);
        ...
        if (ps_sms_options('enable_super_admin_sms', 'sms_super_admin_settings', 'on') == 'on') {
            ...
            if (in_array($new_status, $super_admin_order_status)) {
                ...
                $super_admin_response_sms = WoocommerceIR_Gateways_SMS::init()->sendAsanak($super_admin_sms_data);
            ...
            if (!empty($product_admin_numbers_sms) && count($product_admin_numbers_sms) > 0) {
                foreach ((array) $product_admin_numbers_sms as $number => $vendor_items) {
                    if (strlen($number) > 5) {
                        ...
                        $admin_response_sms = WoocommerceIR_Gateways_SMS::init()->sendAsanak($admin_sms_data);
            ...
            <script type="text/javascript">
                jQuery(document).ready(function(){
                    jQuery("#order_status").change(function(){
                        jQuery("#asanak_sms_textbox").html( "<img src=\"<?php echo PS_WOO_SMS_PLUGIN_PATH ?>/assets/images/ajax-loader.gif\" />" );
                        ...
                        jQuery.ajax({
                            ...
                            success : function( response ) {
                                jQuery("#asanak_sms_textbox").html( response );
                            }
                        });
                    });
                });
            </script>
            <p class="form-field form-field-wide"  id="asanak_sms_textbox_p" >
                <span id="asanak_sms_textbox" class="asanak_sms_textbox"></span>
            </p>
            <?php
        }
    }
    ...
    function changeSmsText()
    {
        ...
        if (defined('DOING_AJAX') && DOING_AJAX) {
            ...
            echo '<input type="hidden" name="shop_order_asanak" value="true" />';
            die();
        } else {
