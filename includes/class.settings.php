...
class WoocommerceIR_Settings_SMS
{
    ...
    function admin_init()
    {
        ...
        if (get_option('redirect_to_woo_sms_about_page_check') != 'yes') {
            ob_start();
            if (!headers_sent()) {
                wp_redirect(admin_url('index.php?page=about-WoocommercePluginAsanak'));
            } else {
    ...
    function adminMenu()
    {
        if (defined('PERSIAN_WOOCOMMERCE_VERSION') && version_compare(PERSIAN_WOOCOMMERCE_VERSION, '2.4.9', '>'))
            add_submenu_page('persian-wc', 'تنظیمات پیامک', 'تنظیمات پیامک', 'manage_woocommerce', 'WoocommercePluginAsanak', array($this, 'settingPage'));
        else
            add_submenu_page('woocommerce', 'تنظیمات پیامک', 'تنظیمات پیامک', 'manage_woocommerce', 'WoocommercePluginAsanak', array($this, 'settingPage'));

        if (get_option('redirect_to_woo_sms_about_page') != 'yes')
            add_submenu_page('index.php', 'درباره پیامک ووکامرس', 'پیامک ووکامرس', 'read', 'about-WoocommercePluginAsanak', array($this, 'aboutPage'));
    }
        ...
        if (current_user_can('manage_woocommerce') && is_admin_bar_showing()) {
            $wp_admin_bar->add_menu(
                array(
                    ...
                    'href' => admin_url('admin.php?page=WoocommercePluginAsanak&send=true'),
                )
            );
        }
    }
            ...
            'sms_main_settings' => apply_filters(
                ...
                array(
                    ...
                    array(
                        ...
                        'label' => 'ارسال از طریق الگو',
                        'desc' => 'در تعریف الگوهای در پنل آسانک ، از شورت کد های ووکامرس استفاه کرده و نام الگو را بجای متن پیامک قرار دهید',
                        'type' => 'checkbox',
                    ),
                    array(
                        'name' => 'header_1',
                    ...
                    array(
                        ...
                        'label' => 'نام کاربری آسانک',
                        'type' => 'text',
                    ),
                    array(
                        'name' => 'persian_woo_sms_password',
                        'label' => 'رمز عبور آسانک',
                        'type' => 'text',
                    ),
                    array(
                    ...
                    array(
                        'name' => 'persian_woo_sms_apiSecret',
                        'label' => 'Api Secret پنل آسانک (برای ارسال از طریق الگو)',
                        'type' => 'text',
                    ),
                )
            ),
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] == 'WoocommercePluginAsanak')
            return ' این افزونه به صورت رایگان از سوی <a href="https://asanak.com/" target="_blank">آسانک</a> ارائه شده است .';
        return $text;
    }
