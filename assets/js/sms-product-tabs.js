jQuery(document).ready(function() {
		...
		clone_container.children( 'p' ).each(function() {
			...
			jQuery( '.new_duplicate_row_sms' ).find('input').each(function() {
				if ( jQuery(this).is('input[name="hidden_duplicator_row_title"]') ) {
					jQuery(this).attr( 'name' , '_asanak_wc_custom_repeatable_product_tabs_tab_title_'+new_count ).attr( 'id' , '_asanak_wc_custom_repeatable_product_tabs_tab_title_'+new_count ).parents('p').addClass('_asanak_wc_custom_repeatable_product_tabs_tab_title_'+new_count+'_field').removeClass('hidden_duplicator_row_title_field_sms').find('label').removeAttr('for').attr('for','_asanak_wc_custom_repeatable_product_tabs_tab_title_'+new_count+'_field');
				}
			});
			
			jQuery( '.new_duplicate_row_sms' ).find('select').each(function() {
				if ( jQuery(this).is('select[name="hidden_duplicator_row_content[]"]') ) {
					jQuery(this).attr( 'name' , '_asanak_wc_custom_repeatable_product_tabs_tab_content_'+new_count+'[]' ).attr( 'id' , '_asanak_wc_custom_repeatable_product_tabs_tab_content_'+new_count ).parents('p').addClass('_asanak_wc_custom_repeatable_product_tabs_tab_content_'+new_count+'_field').removeClass('hidden_duplicator_row_content_field_sms').find('label').removeAttr('for').attr('for','_asanak_wc_custom_repeatable_product_tabs_tab_content_'+new_count+'_field');
				}
			});
			...
			jQuery( '.new_duplicate_row_sms' ).first().before( '<div class="asanak-woo-custom-tab-divider"></div>' );
			
		});
		
		move_tab_content_buttons.clone().insertAfter( jQuery( '.asanak-woo-custom-tab-divider').last() ).addClass( 'last-button-holder-sms' );	
	...
	*/
	...
	/* 
		...
		- slide out display
	*/
	jQuery( '.asanak-tabs-how-to-toggle' ).on( 'click' , function( e ) {
		jQuery( '.asanak-woo-tabs-hidden-how-to-info' ).slideToggle( 'fast' );
	});
